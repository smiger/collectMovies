...
class CustomPage
{
    ...
    public static function getSelfPageView($nowPage, $totalPage, $baseUrl, $search)
    {
        ...
        if ($nowPage <= 1) {
            $nowPage = 1;
            $pageLastStr = '<li class="page-item disabled"><span class="page-link">«</span></li>';
        }
        if ($nowPage >= $totalPage) {
            $nowPage = $totalPage;
            $pageNextStr = '<li class="page-item disabled"><span class="page-link">»</span></li>';
        }
//        $search = array();
        ...
        if($totalPage > 7 && $totalPage - $nowPage > 3){
            $tmp = $nowPage + 5;
            if($tmp > $totalPage){
                $tmp = $totalPage;
            }
            ...
            $searchStr = self::arrayToSearchStr($search);
            $url = $baseUrl . '?' . $searchStr;
            $pageTemp .= self::getPageLinkWrapper($url, "...");

            $search['nowPage'] = $totalPage;
            $searchStr = self::arrayToSearchStr($search);
            $url = $baseUrl . '?' . $searchStr;
            $pageTemp .= self::getPageLinkWrapper($url, $totalPage);
        }
    ...
    public static function getPageRange($nowPage, $totalPage)
    {
        ...
        if ($totalPage <= 7) {
            ...
            if (($lengthLeft < 3) && ($lengthRight < 3)) {
                $returnArray = [];
            } elseif (($lengthLeft < 3) && ($lengthRight > 3)) {
                for ($i = 1; $i <= 7; $i++) {
                    $returnArray[] = $i;
                }
            } elseif (($lengthLeft > 3) && ($lengthRight < 3)) {
